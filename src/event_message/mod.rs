...
pub mod parse_signed_event {
    ...
    fn json_message(s: &str) -> nom::IResult<&str, EventMessage> {
        let mut stream =
            serde_json::Deserializer::from_slice(s.as_bytes()).into_iter::<EventMessage>();
        match stream.next() {
    ...
    fn cbor_message(s: &str) -> nom::IResult<&str, EventMessage> {
        let mut stream =
            serde_cbor::Deserializer::from_slice(s.as_bytes()).into_iter::<EventMessage>();
        match stream.next() {
            Some(Ok(event)) => Ok((&s[stream.byte_offset()..], event)),
            _ => Err(nom::Err::Failure((s, ErrorKind::IsNot))),
        }
    ...
    fn signatures(s: &str) -> nom::IResult<&str, Vec<AttachedSignaturePrefix>> {
        let (rest, t) = tuple((sig_count, many0(signature)))(s)?;
        if t.0 as usize != t.1.len() {
            Err(nom::Err::Failure((s, ErrorKind::Count)))
        } else {
            Ok((rest, t.1))
        }
    }

    fn signed_message(s: &str) -> nom::IResult<&str, SignedEventMessage> {
