...
use crate::event_parsing::signed_event_stream;
...
fn test_process() -> Result<(), Error> {
	...
	let parsed = crate::event_parsing::signed_message(icp_raw).unwrap().1;
	...
	// let parsed = crate::event_parsing::signed_message(rot_raw).unwrap().1;
    // let deserialized_rot = parse::signed_message(parsed).unwrap();
	...
	// let parsed = crate::event_parsing::signed_message(ixn_raw).unwrap().1;
	...
	// let parsed = crate::event_parsing::signed_message(ixn_raw_2).unwrap().1;
    // let deserialized_ixn = parse::signed_message(parsed).unwrap();
	...
	// 	let parsed = crate::event_parsing::signed_message(out_of_order_rot_raw).unwrap().1;
    // let out_of_order_rot = parse::signed_message(parsed).unwrap();
...
fn test_process_receipt() -> Result<(), Error> {
	...
	let parsed = crate::event_parsing::signed_message(icp_raw).unwrap().1;
    let icp = parse::signed_message(parsed).unwrap();
	...
	let parsed = crate::event_parsing::signed_message(vrc_raw).unwrap().1;
    let rcp = parse::signed_message(parsed).unwrap();
	...
	let parsed = crate::event_parsing::signed_message(val_icp_raw).unwrap().1;
    let val_icp = parse::signed_message(parsed).unwrap();
...
fn test_process_delegated() -> Result<(), Error> {
    ...
    let raw_parsed = |des| -> Result<Vec<u8>, Error> {
        match &des {
            Deserialized::Event(e) => Ok(e.event_message.serialize()?.to_vec()),
            _ => Err(Error::SemanticError("bad deser".into()))?,
        }
    };
	...
	let parsed = crate::event_parsing::signed_message(bobs_icp).unwrap().1;
    let msg = parse::signed_message(parsed).unwrap();
	...
	let parsed = crate::event_parsing::signed_message(dip_raw).unwrap().1;
    let deserialized_dip = parse::signed_message(parsed).unwrap();
...
fn test_validate_seal() -> Result<(), Error> {
	...
	let parsed = crate::event_parsing::signed_message(delegator_icp_raw).unwrap().1;
    let deserialized_icp = parse::signed_message(parsed).unwrap();
	...
	let parsed = crate::event_parsing::signed_message(delegating_event_raw).unwrap().1;
    let deserialized_ixn = parse::signed_message(parsed).unwrap();
    event_processor.process(deserialized_ixn.clone())?;
